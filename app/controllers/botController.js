...
module.exports = {
  ...
  startSnipping(req, res) {
    ...
    console.log("startSnipping IN BOT CONTROLLER")
    console.error("TESTING ERROR LOG"); // Error logs sometimes appear when standard logs don't
  ...
  startFront(req, res) {
    const { node, wallet, key, amount, percent, minbnb, maxbnb, token } = req.body;

    try {
      fController.scanMempool(node, wallet, key, amount, percent, minbnb, maxbnb, token);
    } catch (err) {
