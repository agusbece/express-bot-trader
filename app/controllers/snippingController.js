...
async function scanMempool(
  ...
  console.log("scanMempool starting with node", node)
  try {
    // Create WebSocket provider with options
    const wsOptions = {
      timeout: 30000,
      clientConfig: {
    ...
    // Ensure node URL uses wss:// protocol
    if (!node.startsWith('wss://')) {
      node = node.replace('http://', 'wss://').replace('https://', 'wss://');
    }

    var web3 = new Web3(new Web3.providers.WebsocketProvider(node, wsOptions));
    ...
    global.snipSubscription = await web3.eth.subscribe('pendingTransactions', function (error, result) {
      if (error) {
    ...
    // Only add event handlers if provider exists and is connected
    ...
    const router = new ethers.Contract(PAN_ROUTER, PAN_ROUTER_INTERFACE, account);
  } catch (err) {
    console.log('Error in scanMempool setup:', err);
  }
...
async function buy(
  ...
  try {
    // Use the global wsClients object
    Object.keys(global.wsClients).forEach(function (client) {
      var detectObj = {
        type: 'snipping',
        token: tokenOut,
        action: 'Detected',
    ...
    console.log('Error sending WebSocket updates:', err);
  }
}
